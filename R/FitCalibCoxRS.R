...
FitCalibCoxRS <- function(w, w.res, Z, hz.times, n.int = 5, order = 2 , event)
{
r <- sum(event)
...
for (j in 1:r)
{
  ...
  if (n.int<2) {
    fit.cox.point <- FitCalibCox <- function(w = w, w.res = w.res, Z = Z, hz.times = hz.times, n.int = n.int, order = order)
